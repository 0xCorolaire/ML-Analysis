...
processImage = function(img) {
  # Resize
  output = resizeImage(img, width = 100, height = 100, method = 'bilinear')
  # Scaling
...
for (i in 1:nb_flowers) {
  ...
  img = readImage(path)
  img = processImage(img)
  x_train[[i + nb_cars + nb_cats]] = img
  y_train[i + nb_cars + nb_cats] = 2
}
...
model %>% 
  ...
  # Outputs from dense layer are projected ono 3 unit output layer
  layer_dense(3) %>%
